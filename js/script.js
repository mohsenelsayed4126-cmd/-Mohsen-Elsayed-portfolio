...
/* ── SIDEBAR ── */
const ham=document.getElementById('ham'),sb=document.getElementById('sidebar'),nov=document.getElementById('nov');
const openSB=()=>{sb.classList.add('open');nov.classList.add('active');ham.classList.add('open')};
const closeSB=()=>{sb.classList.remove('open');nov.classList.remove('active');ham.classList.remove('open')};
ham.addEventListener('click',()=>sb.classList.contains('open')?closeSB():openSB());
nov.addEventListener('click',closeSB);
document.querySelectorAll('.nl a').forEach(a=>a.addEventListener('click',()=>{if(innerWidth<=920)closeSB()}));

/* ── THEME ── */
const html=document.documentElement;
document.getElementById('ttog').addEventListener('click',()=>{
...
new IntersectionObserver(entries=>{
  entries.forEach(e=>{
    if(e.isIntersecting&&!ba){ba=true;fills.forEach(b=>{b.style.width='0';requestAnimationFrame(()=>requestAnimationFrame(()=>{b.style.width=b.dataset.width}))})}
  });
},{threshold:.15}).observe(document.getElementById('skills'));

/* ── COUNTER ── */
function countUp(el,t){let c=0;const s=Math.ceil(t/40);const iv=setInterval(()=>{c+=s;if(c>=t){c=t;clearInterval(iv)}el.textContent=c+'+'},40)}
new IntersectionObserver(entries=>{
  entries.forEach(e=>{
    if(e.isIntersecting){e.target.querySelectorAll('[data-count]').forEach(el=>countUp(el,+el.dataset.count));rvObs.unobserve(e.target)}
  });
...
new IntersectionObserver(entries=>{
  entries.forEach(e=>{
    if(e.isIntersecting){nls.forEach(a=>a.classList.remove('active'));const a=document.querySelector(`.nl a[href="#${e.target.id}"]`);if(a)a.classList.add('active')}
  });
},{threshold:.3,rootMargin:'-60px 0px -38% 0px'}).observe(document.getElementById('home'));
secs.forEach(s=>new IntersectionObserver(entries=>{
  entries.forEach(e=>{if(e.isIntersecting){nls.forEach(a=>a.classList.remove('active'));const a=document.querySelector(`.nl a[href="#${e.target.id}"]`);if(a)a.classList.add('active')}});
},{threshold:.3,rootMargin:'-60px 0px -38% 0px'}).observe(s));

/* ── BUTTONS ── */
document.getElementById('hireBtn').addEventListener('click',()=>{document.getElementById('contact').scrollIntoView({behavior:'smooth'});if(innerWidth<=920)closeSB()});
document.getElementById('vProj').addEventListener('click',()=>document.getElementById('projects').scrollIntoView({behavior:'smooth'}));
...
function spawnP(){
  ...
  const c=cols[Math.floor(Math.random()*cols.length)],dur=Math.random()*14+8;
...
if(!('ontouchstart' in window)){setInterval(spawnP,1100);for(let i=0;i<8;i++)setTimeout(spawnP,i*250)}
